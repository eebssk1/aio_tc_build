...
../configure --target=x86_64-linux-gnu --prefix=/usr --enable-pgo-build=lto --enable-nls --enable-plugins --enable-multilib --enable-compressed-debug-sections=all --enable-checking=release --enable-new-dtags --disable-gdb --disable-gdbserver --disable-gprofng --with-system-zlib --with-zstd || exit 255
