...
curl -L "https://github.com/eebssk1/mingw-crt-build/releases/download/4b06d704/mingw-crt.tgz" | tar -zxf -

echo current utc time 1 is $(date -u)

cd m_binutils; mkdir build; cd build

../configure --prefix=$CUR/out --target=x86_64-w64-mingw32 --enable-64-bit-bfd --enable-nls --disable-rpath --disable-multilib --enable-install-libiberty --enable-plugins --enable-deterministic-archives --disable-werror --enable-lto --with-system-zlib --with-zstd --disable-gdb --disable-gprof --disable-gprofng || exit 255
...
../configure --prefix=$CUR/out --target=x86_64-w64-mingw32 --enable-checking=release --with-local-prefix=$CUR/out/x86_64-w64-mingw32/local  --with-arch=$TAC --with-tune=skylake --with-gcc-major-version-only --with-default-libstdcxx-abi=new --disable-vtable-verify --enable-plugin  --enable-libatomic --enable-threads=posix --enable-graphite --enable-fully-dynamic-string --enable-libstdcxx-filesystem-ts --enable-libstdcxx-time --disable-libstdcxx-pch --enable-lto --enable-libgomp --disable-multilib --disable-rpath --enable-nls --disable-werror --disable-symvers --disable-libstdcxx-debug --enable-languages=c,c++,lto --disable-sjlj-exceptions || exit 255
make -j$(($N+2)) all MAKEINFO=true || exit 255
