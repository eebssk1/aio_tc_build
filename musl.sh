...
export CFLAGS="-U_FORTIFY_SOURCE -D_FORTIFY_SOURCE=1 -D__BUILD_NO_CON__ -Wa,-O2 -flto-compression-level=1 -fprofile-partial-training -flto=2 -frandom-seed=1 -I/usr/local/include  @$CUR/gccflags -Wno-error=maybe-uninitialized"
...
../configure --target=x86_64-linux-musl --prefix=$CUR/out --enable-nls --enable-plugins --enable-compressed-debug-sections=all --enable-checking=release --enable-deterministic-archives --enable-new-dtags --disable-gdb --disable-gdbserver --disable-gprofng --with-system-zlib --with-zstd || exit 255
